...
function limpar() {
  ...
  function preencher(valor) {
    ...
    // Evitar repetições.
    var displayResultado = document.getElementById('visor').value;
    var nResultado = displayResultado.replace('++', '+').replace('1(', '1').replace(')1', ')')
      ...
      .replace('))', ')')
      .replace('..', '.')
    document.getElementById('visor').value = nResultado;
  ...
  function calcular() {
    ...
    var nvisor = visor.charAt(visor.length - 1);
    if (nvisor == '+' || nvisor == '-') {
...
//     var nValor = valor.lastIndexOf('+');
  
//     var xal = valor.substring(nValor);
